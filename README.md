...
增加辅具尺寸不适复评。老人使用护理床或轮椅后反馈不适时，平台要求上传使用照片、身高体重、房间尺寸和照护人说明。评估师判断是操作问题、尺寸不合还是病情变化；结论会决定换型、培训或暂停租赁。换型后要重新确认门宽和照护人操作。辅具出库要记录老人身体状况、适配建议和家属确认，维修时才能判断是否误用。复评后家属会看到换型或培训的具体原因。

### 追加需求（第三轮，原文）

增加维修上门排程。辅具异响、刹车失灵或气垫漏气时，平台根据风险等级、老人是否独居、维修员距离和备件库存安排上门。高风险故障会优先处理，并通知照护人临时采取安全措施。维修员到场后需要回传试用结果。回收检测需区分正常磨损、配件缺失和护理员操作问题，押金结算才有依据。维修排程会同步提示照护人临时替代办法。
...
9. **尺寸不适复评**（护理床/轮椅）：`staff` 对在租订单发起复评（或反馈处置选「尺寸复评」自动生成）→ `family` 上传使用照片、身高体重、房间尺寸、照护人说明 → `assessor` 复评判定 **操作问题/尺寸不合/病情变化**，决定 **培训/换型/暂停租赁**（结论原因家属可见）→ 换型由 `staff` 执行，**必须重新确认门宽与照护人操作**，换型后需重新登记出库适配。
10. **出库适配与误用判定**：`warehouse`/`staff` 在租赁详情「登记出库适配」（老人身体状况+适配建议）→ 家属确认；维修完成时维修页展示该记录，据此判定「正常磨损/家属误用」并留痕。
11. **维修上门排程**：反馈异响/刹车失灵/气垫漏气等故障并处置为维修后，平台自动按 **风险等级 + 老人是否独居 + 维修员距离 + 备件库存** 计算优先级（紧急/优先/常规）与上门时间，生成维修单；同时向照护人推送 **临时安全措施** 与 **临时替代办法**（家属在维修页可见）。`warehouse` 到场后 **回传试用结果**：试用仍有问题则退回待上门重新排程；试用正常则完成维修（费用入账、备件出库扣减）。
12. **回收检测定责**：消毒质检时区分 **正常磨损 / 配件缺失 / 护理员操作问题**；后两者可录入扣款金额，自动冲减待退押金并生成「检测扣款」费用，作为押金结算依据。
...
| 复评 | `GET/POST /api/fit-reviews`、`POST /api/fit-reviews/{id}/info|review|exchange` |
| 维修 | `GET /api/repairs`、`GET /api/repairs/spare-parts`、`POST /api/repairs/{id}/schedule|start|finish`（finish 含试用结果与误用判定） |
| 费用 | `GET /api/payments`、`POST /api/payments/{id}/pay|refund` |
| 补贴 | `GET/POST /api/subsidies`、`POST /api/subsidies/{id}/review|writeoff` |
| 附件 | `POST /api/files`（上传）、`GET /api/files/{name}` |
...
- 尺寸复评：`family` 在「尺寸复评」页可见种子复评单（轮椅待上传资料）；上传照片/身高体重/房间尺寸后，`assessor` 复评给结论，`staff` 执行换型（强制门宽复测+照护人确认）；租赁详情可登记出库适配并由家属确认，维修完成时可据此判定误用。
- 维修排程：提交「刹车失灵」反馈并处置为维修 → 维修单自动排程（紧急优先级、最近维修员、备件库存、安全措施与替代办法）；「开始上门」→「完成维修」须回传试用结果；备件库存随之扣减。
- 回收检测：结案回收的辅具质检时选「配件缺失/护理员操作问题」并填扣款 → 待退押金自动冲减，费用中心出现「检测扣款」记录。
